...
get_north_west_london_code <- function() {
  ...
  msoa_population <- read.csv(here("src/src_data/Population_Statistics%2C_England_(MSOA_level).csv")) %>% 
    janitor::clean_names() 
  
  lsoa_icb <-  readxl::read_excel(here("src/src_data/LSOA11_LOC22_ICB22_LAD22_EN_LU.xlsx")) %>% 
    janitor::clean_names()
  
  ward_lsoa <- read.csv(here("src/src_data/lsoa_21_ward_22_lad_22.csv")) %>% 
    janitor::clean_names()
  
  nwl_geography_codes <- read.csv(here("src/src_data/lsoa_2011_lsoa_2021_msoa_lad_lookup_for_nwl.csv")) %>% 
...
get_geography_codes <- function() {
  ...
  geography_codes <- read.csv(here("src/src_data/LSOA_(2011)_to_LSOA_(2021)_to_Local_Authority_District_(2022)_Lookup_for_England_and_Wales_(Version_2).csv")) %>% 
    janitor::clean_names() %>% 
    #dplyr::left_join(msoa_population %>% distinct(msoa11cd, all_ages), by="msoa11cd") %>% 
    dplyr::left_join(lsoa_icb %>%  select(lsoa11cd, icb22cd, icb22cdh, icb22nm),  by=c("lsoa11cd"="lsoa11cd"))
...
ingest_shape_date <- function(geography, filter_codes, all_codes) {
  ...
  if (geography=="lsoa") {
    shape <- sf::st_read(here("src/src_data/LSOA_2021_EW_BGC.shp"),  quiet = TRUE) %>% 
      janitor::clean_names() %>% 
  ...
  if (geography=="lsoa_11") {
    shape <- sf::st_read(here("src/src_data/LSOA_2011_EW_BFC_V3.shp"),  quiet = TRUE) %>% 
      janitor::clean_names() %>% 
  ...
  if (geography=="msoa") {
    shape <- sf::st_read(here("src/src_data/infuse_msoa_lyr_2011.shp"),  quiet = TRUE) %>% 
      janitor::clean_names() %>% 
  ...
  if (geography=="ward") {
    shape <- sf::st_read(here("src/src_data/WD_DEC_2022_UK_BSC.shp"),  quiet = TRUE) %>% 
      janitor::clean_names() %>% 
